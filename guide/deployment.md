...
サーバー側のAPIキーや `CUSTOM_API_*` を公開URLのAPIルートから使う場合は、`AITUBERKIT_SERVER_SECRET_ACCESS_MODE` を設定してください。公開デモでは `demo` と `AITUBERKIT_ALLOWED_ORIGINS`、外部アプリや管理用途では `protected` と `AITUBERKIT_SERVER_SECRET_TOKEN` の利用を推奨します。`unprotected` は公開URLでは推奨されません。

利用者が各自のAPIキーをブラウザ設定から使う通常構成では、この設定による追加対応は基本的に不要です。
:::
