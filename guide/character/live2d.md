...
::: warning Live2D機能はデフォルトで無効です
Live2D機能はライセンス上の制約により、デフォルトでは無効になっています。Live2D機能を有効にするには、以下の設定が必要です：

1. Live2D Cubism Core のスクリプトファイル（`live2dcubismcore.min.js`）を `public/scripts/` に配置してください
2. 環境変数 `NEXT_PUBLIC_LIVE2D_ENABLED=true` を `.env` ファイルに設定してください

設定後、アプリケーションを再起動するとLive2Dモデルが使用可能になります。Live2D機能が未有効の状態でキャラクター設定画面を開くと、上記のセットアップ手順がガイドとして表示されます。

商用利用の場合はLive2D社のライセンスが必要です。
:::

![Live2Dセットアップガイド](/images/live2d_setup_guide_r5d4c.webp)
