...
キャラクター設定画面では、AIキャラクターの名前、使用するモデル（VRM、Live2D、またはPNGTuber）、キャラクタープロンプトを設定することができます。
...
キャラクターのモデルタイプを「VRM」「Live2D」「PNGTuber」から選択できます。それぞれのモデルタイプによって、設定項目が変わります。

- [VRMの設定](./vrm.md)
- [Live2Dの設定](./live2d.md)
- [PNGTuberの設定](./pngtuber.md)
...
```bash
# VRM/Live2Dキャラクター位置設定
NEXT_PUBLIC_CHARACTER_POSITION="0,0,0,1"
NEXT_PUBLIC_CHARACTER_ROTATION="0,0,0"
NEXT_PUBLIC_FIXED_CHARACTER_POSITION="false"
```

- `NEXT_PUBLIC_CHARACTER_POSITION` - カメラ/モデルの位置（x,y,z,zoom）
- `NEXT_PUBLIC_CHARACTER_ROTATION` - カメラ/モデルの回転（x,y,z）
- `NEXT_PUBLIC_FIXED_CHARACTER_POSITION` - 位置を固定するかどうか（true/false）
