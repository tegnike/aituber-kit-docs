...
简易设置会把开始使用 AITuberKit 所需的主要项目集中在一个设置页中。需要更详细的调整时，可以通过各部分的链接跳转到对应类别。

![简易设置](/images/easy_setup_h7k2p.webp)

## 设置画面结构

设置画面由左侧类别导航、顶部搜索与状态显示、右侧设置内容组成。

- **搜索**: 可通过设置名称或相关关键词查找项目
- **显示语言**: 可从顶部下拉框切换界面语言
- **状态标签**: 可确认当前 AI 服务、语音引擎和已启用的模式
- **类别**: 分为开始、对话、语音/输入、直播/显示、自动化和其他

## 1. 基本信息

首先设置显示名称和语言。

- **角色名称**: 显示在画面上的 AI 角色名称
- **用户显示名**: 用于聊天记录和提示词的用户名称
- **语言设置**: UI 显示和语言相关设置的基准语言
- **显示回答栏**: 聊天记录未打开时，在画面上显示 AI 回复
- **在回答栏显示角色名**: 在回答栏中显示角色名称

背景、主题和显示控制可在[基本设置](/zh/guide/basic-settings)中调整。
...
直播、图片、幻灯片、自动发言等附加功能，可在需要时从各类别中设置。

- [角色设置](/zh/guide/character/common)
