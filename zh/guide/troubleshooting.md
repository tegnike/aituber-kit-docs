...
::: warning 症状
屏幕共享权限已授予，但右上角应显示的小窗口没有出现。游戏解说的倒计时会继续，但捕获失败，因此不会生成解说。
:::
...
- 屏幕共享预览被隐藏或已停止。
- 浏览器中的屏幕共享已经结束。
- 当前模型不支持多模态输入，或启用了互斥模式。

### 解决方法

1. 从菜单中的“Capture”按钮开始屏幕共享，并选择共享对象。
2. 确认右上角显示小窗口。如果已隐藏，请使用显示按钮恢复。
3. 在游戏解说设置中，确认选择了支持多模态的模型。
4. 确认未启用实时API、音频模式、外部连接、空闲、人感检测或幻灯片等互斥模式。
5. 检查浏览器的屏幕共享指示器。如果共享已经结束，请重新开始 Capture。
